...
from beb_chargers.opt.benders_charge_scheduling import solve_with_benders
from beb_chargers.scripts.script_helpers import build_trips_df
from beb_chargers.scripts.charge_scheduling_with_simulation import \
    build_scheduling_inputs, add_realtime_durations, load_realtime_summary
from numpy.random import default_rng
from beb_chargers.vis import plot_trips_and_terminals
...
def run_multi_charger_case(
    ...
    # ]
    if scenario == 'a':
        locs_df = pd.read_csv('../data/tre_sites.csv', index_col=0)
    elif scenario == 'b':
        locs_df = pd.read_csv('../data/tre_sites_b.csv', index_col=0)
    else:
...
if __name__ == '__main__':
    ...
    soln_df_b.to_csv('tre_b_soln.csv')
